...
describe('directive: ma-embedded-list-column', function () {
    ...
    angular.module('testapp_EmbeddedListColumn', []).directive('maEmbeddedListColumn', directive).service('NgAdminConfiguration', function () {
        return function () {
            return {};
        };
    });
    ...
    it('should contain a datagrid', function () {
        scope.field = new EmbeddedListField().targetFields([new Field('num'), new Field('name')]);
        scope.value = [{num: 1, name: 'foo', dummy: 0}, {num: 2, name: 'bar', dummy: 1}];
        var element = $compile(directiveUsage)(scope);
        scope.$digest();
        expect(element.children()[0].nodeName).toBe('MA-DATAGRID');
        var datagridScope = element.children().eq(0).scope();
    ...
    it('should sort the list according to the sortField', function () {
        scope.field = new EmbeddedListField().targetFields([new Field('num'), new Field('name')]).sortField('num').sortDir('DESC');
        scope.value = [{num: 1, name: 'foo', dummy: 0}, {num: 2, name: 'bar', dummy: 1}];
        var element = $compile(directiveUsage)(scope);
        scope.$digest();
        var datagridScope = angular.element(element.children()[0]).scope();
        expect(datagridScope.entries.length).toBe(2);
    ...
    it('should filter the list according to the permanentFilters', function () {
        scope.field = new EmbeddedListField().targetFields([new Field('num'), new Field('name')]).permanentFilters({name: 'foo'});
        scope.value = [{num: 1, name: 'foo', dummy: 0}, {num: 2, name: 'bar', dummy: 1}];
        var element = $compile(directiveUsage)(scope);
        scope.$digest();
        var datagridScope = angular.element(element.children()[0]).scope();
...
//# sourceMappingURL=maEmbeddedListColumnSpec.js.map