describe('textAngular', function () {
    ...
    describe('Toolbar interaction functions work', function () {
        ...
        describe('ta-element-select event', function () {
            ...
            it('fires correctly when filter returns true', inject(function (taTools) {
                taTools.testbutton.onElementSelect.filter = function () {
                    return true;
                };
                var triggerElement = editorScope.displayElements.text.find('i');
                triggerElement.triggerHandler('click');
    ...
    describe('File Drop Event', function () {
        ...
        it('should respect the function set in the attribute', inject(function ($compile, $rootScope) {
            ...
            if (jQuery === angular.element)
                textAngularManager.retrieveEditor('test').scope.displayElements.text.triggerHandler({type: 'drop', originalEvent: {dataTransfer: {files: ['test'], types: ['files']}}});
            else
                textAngularManager.retrieveEditor('test').scope.displayElements.text.triggerHandler('drop', {originalEvent: {dataTransfer: {files: ['test'], types: ['files']}}});
            $rootScope.$digest();
        ...
        it('handler not fired when no files and no errors', inject(function ($compile, $rootScope, taOptions) {
            ...
            element = $compile('<text-angular name="test" ng-model="html"></text-angular>')($rootScope);
            if (jQuery === angular.element)
                textAngularManager.retrieveEditor('test').scope.displayElements.text.triggerHandler({type: 'drop', originalEvent: {dataTransfer: {files: [], types: ['url or something']}}});
            else
                textAngularManager.retrieveEditor('test').scope.displayElements.text.triggerHandler('drop', {originalEvent: {dataTransfer: {files: [], types: ['url or something']}}});
            $rootScope.$digest();
            expect(testvar).toBe(false);
            expect(function () {
                if (jQuery === angular.element)
                    textAngularManager.retrieveEditor('test').scope.displayElements.text.triggerHandler({type: 'drop', originalEvent: {dataTransfer: {}}});
                else
                    textAngularManager.retrieveEditor('test').scope.displayElements.text.triggerHandler('drop', {originalEvent: {dataTransfer: {}}});
            }).not.toThrow();
            expect(function () {
                if (jQuery === angular.element)
                    textAngularManager.retrieveEditor('test').scope.displayElements.text.triggerHandler({type: 'drop', originalEvent: {}});
                else
                    textAngularManager.retrieveEditor('test').scope.displayElements.text.triggerHandler('drop', {originalEvent: {}});
            }).not.toThrow();
        }));
    ...
    describe('handles the ta-paste event correctly', function () {
        ...
        it('allows conversion of html', inject(function ($window, _$rootScope_, $compile, $document, $timeout) {
            ...
            sel.setSingleRange(range);
            triggerEvent('paste', element, {originalEvent: {clipboardData: {types: ['text/html'], getData: function () {
                            return '<font>Test 4 Content</font>';
                        }}}});
            $timeout.flush();
            $rootScope.$digest();
