...
(function (mod) {
    ...
    function resolveDoc(ts, id) {
        if (typeof id == "string")
            return ts.docs[id];
        if (id instanceof CodeMirror)
            id = id.getDoc();
        if (id instanceof CodeMirror.Doc)
            return findDoc(ts, id);
    }
    ...
    function findContext(doc, data) {
        ...
        while (cursor.findNext()) {
            ...
            if (!dist)
                dist = Math.abs(from.ch - start.ch);
            if (dist < nearestDist) {
                nearest = from;
                nearestDist = dist;
            }
    ...
    function rename(ts, cm) {
        ...
        if (!/\w/.test(token.string))
            return showError(ts, cm, "Not at a variable");
