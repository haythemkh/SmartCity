...
Chain.prototype.push = function (o, link) {
    if (this.nodes.length > 0 && link)
        this.nodes.push(new ChainNode(o, link));
    else
...
Chain.prototype.unshift = function (o, link) {
    if (this.nodes.length > 0 && link)
        this.nodes[0].link = link;
    this.nodes.unshift(new ChainNode(o));
    this.cursor++;
}

Chain.prototype.get = function () {
    if (this.cursor < 0 || this.cursor > this.nodes.length - 1)
        return null;
...
Chain.prototype.joinLeft = function () {
    ...
    for (var i = 0, l = this.cursor; i < l; i++) {
        if (result && this.nodes[i].link)
            result += this.nodes[i].link;
