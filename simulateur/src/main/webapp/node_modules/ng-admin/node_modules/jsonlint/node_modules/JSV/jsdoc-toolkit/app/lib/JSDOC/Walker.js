    ...
    if (this.token.is("JSDOC")) { // it's a doc comment
        ...
        if (doc.getTag("lends").length > 0) {
            ...
            if (matching)
                ...
                LOG.warn("Mismatched } character. Can't parse code in file " + symbol.srcFile + ".");
            ...
            if (doc.meta == "@+")
                JSDOC.DocComment.shared = doc.src;
            else if (doc.meta == "@-")
                JSDOC.DocComment.shared = "";
            else if (doc.meta == "nocode+")
                JSDOC.Parser.conf.ignoreCode = true;
            else if (doc.meta == "nocode-")
                JSDOC.Parser.conf.ignoreCode = JSDOC.opt.n;
            else
                throw "Unrecognized meta comment: " + doc.meta;
        ...
        if (this.token.is("NAME")) { // it's the name of something
            ...
            // function foo() {}
                ...
                if (doc && doc.getTag("memberOf").length > 0) {
                    ...
                    name = this.namescope.last().alias + "-" + name;
                    if (!this.namescope.last().is("GLOBAL"))
                        isInner = true;
                }
            ...
            // foo: function() {}
                ...
                if (doc && doc.getTag("constructs").length) {
                    ...
                    if (name.indexOf("#") > -1)
                        ...
                        name = this.namescope.last().alias;
...
JSDOC.Walker.prototype.resolveThis = function (name) {
    ...
    // if we are in a function, `this` means the container (possibly the global)
        ...
        if (symbol.alias.match(/(^.*)[#.-][^#.-]+/)) {
            ...
            if (!parent) {
                if (JSDOC.Lang.isBuiltin(parentName))
                    parent = JSDOC.Parser.addBuiltin(parentName);
                else {
