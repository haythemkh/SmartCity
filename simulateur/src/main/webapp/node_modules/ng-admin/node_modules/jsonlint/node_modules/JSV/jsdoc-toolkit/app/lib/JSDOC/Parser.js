...
JSDOC.Parser = {
    ...
    addSymbol: function (symbol) {
        ...
        if (JSDOC.opt.S) {
            if (typeof JSDOC.Parser.secureModules == "undefined")
                JSDOC.Parser.secureModules = {};
        ...
        if (JSDOC.Parser.conf.ignoreAnonymous && symbol.name.match(/\$anonymous\b/))
            return;
        ...
        if (JSDOC.Parser.conf.treatUnderscoredAsPrivate && symbol.name.match(/[.#-]_[^.#-]+$/)) {
            if (!symbol.comment.getTag("public").length > 0)
                symbol.isPrivate = true;
        }

        // -p flag is required to document private things
        if (!JSDOC.opt.p && symbol.isPrivate)
            return; // issue #161 fixed by mcbain.asm
